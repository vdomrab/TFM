...
public class L3_AdvancedAutomation extends L3_FlyingService implements IL3_AdvancedAutomation {
	...
	private boolean handleTakeoffPhase(double pitch) {
	    ...
	    final double ROTATION_SPEED = 77.0; // m/s ≈ 150 knots
	    ...
	    if (this.getFADEC().getCurrentThrust() < 95.0) {
	        this.getFADEC().setTHRUSTPercentage(100.0);
	        logger.info("TAKEOFF: Setting full thrust (100%).");
	        correctionDone = true;
	    }
	...
	private boolean handleLandingPhase(double pitch) {
	    ...
	    boolean onGround = radioAltimeterSensor.isOnGround();  // Detecta contacto con el suelo
	    double currentThrust = this.getFADEC().getCurrentThrust();
	    ...
	    if (onGround) {
	        ...
	        double correctionTime = timeStep * 2.0;
	        double densityFactor = this.AHRSSensor.calculateDensityFactor(this.weatherSensor);
	        ...
	        if (Math.abs(pitchError) < 0.1) {
	            this.getControlSurfaces().setElevatorDeflection(0.0);
	            logger.info("LANDING: On ground, pitch ≈ 0°, elevator neutral.");
	        } else {
	            this.getControlSurfaces().setElevatorDeflection(requiredElevatorDeflection);
	            logger.info(String.format(
	                "LANDING: On ground, correcting pitch to 0° (deflection=%.2f°)",
	                requiredElevatorDeflection
	            ));
	        }
	        correctionDone = true;
	        ...
	        if (currentThrust > 0.0) {
	            ...
	            logger.info("LANDING: On ground, setting thrust to 0%.");
	            correctionDone = true;
	        }
	        ...
	        if (currentAirbrakeDeployment < 0.9) {
	            ...
	            logger.info("LANDING: On ground, deploying airbrakes to 90%.");
	            correctionDone = true;
	        }
	        ...
	        double correctionTimeYaw = timeStep * 2.0;
	    ...
	    return false; // No corrección si no está en tierra
	}
