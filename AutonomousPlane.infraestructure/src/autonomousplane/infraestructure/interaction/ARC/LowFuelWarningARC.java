...
public class LowFuelWarningARC extends AdaptiveReadyComponent implements IAdaptiveReadyComponent {

	public static String PROVIDED_MECHANISM = "provided_service";
	protected IInteractionMechanism mechanism = null;

	public LowFuelWarningARC(BundleContext context, String id) {
		super(context, context.getBundle().getSymbolicName());
		logger = SmartLogger.getLogger(context.getBundle().getSymbolicName());
		this.mechanism = new LowFuelWarning(this.context, id);
	}

	@Override
	public IAdaptiveReadyComponent deploy() {
		((LowFuelWarning)this.mechanism).registerThing();
		return super.deploy();
	}

	@Override
	public IAdaptiveReadyComponent undeploy() {
		((LowFuelWarning)this.mechanism).unregisterThing();
		this.mechanism = null;
